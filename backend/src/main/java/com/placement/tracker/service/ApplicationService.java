...
// Service layer for application operations like apply, update status, and listing.
@Service
@Transactional
public class ApplicationService {
    ...
    // Steps: resolve student from email -> load job/profile -> validate -> save application -> return DTO.
    public ApplicationResponse apply(String email, Long jobId) {
        ...
        boolean alreadyApplied = applicationRepository.findByStudentId(studentId)
                .stream()
                .anyMatch(a -> a.getJobPost().getId().equals(jobId));
        if (alreadyApplied) {
            throw new IllegalArgumentException("Student already applied to this job");
        }
    ...
    // Steps: load application by id -> update status -> save -> return DTO.
    public ApplicationResponse updateStatus(Long appId, AppStatus status) {
    ...
    // Steps: query all applications for a job -> map entities to DTOs -> return list.
    ...
    // Steps: resolve student from email -> query applications -> map to DTO list.
    @Transactional(readOnly = true)
